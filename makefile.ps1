...
function DockerFunc {   #can not just be docker because it creates an infinite loop
    ...
    if ($COMMIT_HASH -eq $TAG) {
        ...
        Invoke-Expression "docker build -f .\DockerFile --platform linux/arm/v7 --build-arg NAME=$NAME-amd64 -t $DOCKER_PKG/$NAME-amd64-dev:$COMMIT_HASH dist"

        Write-Output "Building container $DOCKER_PKG/$NAME-arm-dev:$COMMIT_HASH"
        Invoke-Expression "docker build -f .\DockerFile --platform linux/arm/v7 --build-arg NAME=$NAME-arm -t $DOCKER_PKG/$NAME-arm-dev:$COMMIT_HASH dist"
        ...
        Invoke-Expression "docker build -f .\DockerFile --platform linux/arm/v7 --build-arg NAME=$NAME-amd64 -t $DOCKER_PKG/$NAME-amd64-dev:$TAG dist"

        Write-Output "Building container $DOCKER_PKG/$NAME-arm-dev:$TAG"
        Invoke-Expression "docker build -f .\DockerFile --platform linux/arm/v7 --build-arg NAME=$NAME-arm -t $DOCKER_PKG/$NAME-arm-dev:$TAG dist"
    } elseif ($TAG -match $PRD_TAG_REGEX) {
        Write-Output "Building prd containers with tag $TAG"

        Write-Output "Building container $DOCKER_PKG/$NAME-amd64-dev:$TAG"
        Invoke-Expression "docker build -f .\DockerFile --platform linux/arm/v7 --build-arg NAME=$NAME-amd64 -t $DOCKER_PKG/${NAME}-amd64:$TAG dist"

    	Write-Output "Building container $DOCKER_PKG/${NAME}:$TAG"
    	Invoke-Expression "docker build -f .\DockerFile --platform linux/arm/v7 --build-arg NAME=$NAME-arm -t $DOCKER_PKG/${NAME}-arm:$TAG dist"
    } else {
